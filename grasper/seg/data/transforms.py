...
class ExplodeTarget(object):
    """
    Convert single channel target into volume where number of channels == number of classes
    """
    def __init__(self, num_cl):
        self.num_cl = num_cl

    def __call__(self, arr):
        default = [x for x in np.unique(arr) if x not in range(2*self.num_cl)]
        if len(default) > 1:
            raise Exception('Cannot have more than 1 default value.')
        default = default[0]
        target = np.full((self.num_cl, arr.shape[0], arr.shape[1]), default, dtype=np.uint8)
        for k in range(self.num_cl):
            target[k, ...].flat[arr.flat == k] = 0
        for i, k in enumerate(range(self.num_cl, self.num_cl*2)):
            target[i, ...].flat[arr.flat == k] = 1
        return target
